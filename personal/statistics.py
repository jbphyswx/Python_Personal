...
def calculate_anomalies(data, axis=-1, t=None, baseline_methods=[[['mean']]], date_axis=None):
    """
    ...
    e.g. baselines = [   [[('annual all', 'mean'), {'args':(),'kwargs':{}}],  [(None, 'rolling mean'), {'args':(),'kwargs':{dim:5, 'center':True, min_periods:1}}] ], 
                         'rolling_mean'
                     ]
...
#     def str_split_inclusive(string, delim): return [ _ + delim for _ in string.split(delim) ] # split while keeping the delimiter
    ...
    if d_type is xr.core.dataarray.DataArray: # is xarray DataArray
        ...
        return data.map(lambda x : calculate_anomalies(x, axis=axis, baseline_methods=baseline_methods)) # just call the fcn on itself, mapped to variables (dataarrays) which are handled by above case
    else: # ideally is numpy or dask array, and we create a DataArray and assign this dim to it
        print(('hurr', type(data), data))
        data = xr.DataArray(data)
        if t is None: x = np.arange(np.shape(data)[axis])
        data = data.rename({data.dims[axis]:'t'})         # rename that dimension
        data = data.assign_coords({'t':t})                # assign values for the dimension
        return calculate_anomalies(data, axis=axis, baseline_methods=baseline_methods).data # apply to dataarray, but return the data at the end since we didn't start with a dataarray
    ...
    for baseline in baseline_methods:
        ...
        for action in baseline:                                      # the grouping, e.g. [('annual all', 'mean'), {'args':(),'kwargs':{}}] or 'rolling_mean'
            ...
            if len(action) == 1: [action[0], {'args':(),'kwargs':{}}]# -- add in placeholder for args if we gave none, tuple and dict unpacking
...
#                 baseline = method_fcn(baseline, *args, **kwargs) # apply directly to dataarray
                baseline = xr.apply_ufunc( lambda x:  method_fcn(x, *args, **kwargs), baseline)
            print('done')
